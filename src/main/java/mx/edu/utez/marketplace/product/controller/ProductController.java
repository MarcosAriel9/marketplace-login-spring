    ...
    public ResponseEntity<Message> save(@RequestBody ProductDTO productDTO) {
        ...
        productDTO.getImages().forEach(image ->
                images.add(new ProductImage(image.getId(), image.getName(),
                        Base64.getDecoder().decode(image.getFileBase64().replace(" ", "+"))
                        , null))
        );
        Product product = new Product(productDTO.getName(), productDTO.getDescription(), productDTO.getBrand(),
                productDTO.getPrice(), productDTO.getQuantity(), productDTO.getStatus(), productDTO.getSubcategory(),
                images);
