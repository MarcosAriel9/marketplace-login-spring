...
public class SubcategoryService {
    ...
    public ResponseEntity<Message> findAll() {
        return new ResponseEntity<>(new Message("OK", false,
                subcategoryRepository.findAll()), HttpStatus.OK);
    }
    ...
    public ResponseEntity<Message> findById(long id) {
        if (subcategoryRepository.existsById(id)) {
            return new ResponseEntity<>(new Message("OK", false,
                    subcategoryRepository.findById(id)), HttpStatus.OK);
        }
        return new ResponseEntity<>(new Message("Subcategoría no encontrada", true,
                null), HttpStatus.BAD_REQUEST);
    }

    @Transactional(rollbackFor = {SQLException.class})
    public ResponseEntity<Message> insert(Subcategory subcategory) {
        Optional<Subcategory> existsSubcategory = subcategoryRepository.findByDescription(
                subcategory.getDescription());
        if (existsSubcategory.isPresent()) {
            return new ResponseEntity<>(new Message("La subcategoría ya existe", true,
                    null), HttpStatus.BAD_REQUEST);
        }
        return new ResponseEntity<>(new Message("OK", false,
    ...
    public ResponseEntity<Message> update(Subcategory subcategory) {
        if (!subcategoryRepository.existsById(subcategory.getId())) {
            return new ResponseEntity<>(new Message("La subcategoría no existe", true,
                    null), HttpStatus.BAD_REQUEST);
        }
        return new ResponseEntity<>(new Message("OK", false,
                subcategoryRepository.saveAndFlush(subcategory)), HttpStatus.OK);
